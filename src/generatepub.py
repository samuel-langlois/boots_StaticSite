...
def generate_public(source_dir='static', dest_dir='public'):
    """
    ...
    for i in (os.listdir(dir_path_content)):
        ...
        if os.path.isdir(entry):
            if not os.path.exists(dest):
                os.makedirs(dest)
            generate_pages_recursive(basepath, entry, template_path, os.path.join(dest_dir_path, i))
            
        if os.path.isfile(entry) and entry.endswith('.md'):
            # Generate the page from the markdown file
            if not os.path.exists(dest_dir_path):
                os.makedirs(dest_dir_path)
            generate_page(basepath, entry, template_path, html_dest)
